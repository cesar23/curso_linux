...
{ // Begin class model_mantcateg
      ...
      tm_equipo_componente.id_com = id_componente.id_com  and estado=1   and tm_lastmsg.lat_dd is not null order by gps_date desc ; ";
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $row) {
                ...
                $dif = $hoy - $phpdate;
                if (($dif >= 86400) and ($dif < 172800)) {
                    $atra = 1;
                } else
                    if (($dif >= 172800) and ($dif < 259200)) {
                        $atra = 2;
                    } else
                        if (($dif >= 259200)) {
                            $atra = 3;
    ...
    //--- Función Pública ultimatxunidad
      ...
      tm_asignacion.codigonave = tm_naves.codigonave AND tm_naves.codigonave=$cn and  codigousuario=$idu  order by Fecha desc limit 500;";
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $row) {
                ...
                $phpdate = strtotime(date($fecha_gps));
                $dif = $hoy - $phpdate;
                if (($dif >= 1440) and ($dif < 2880)) {
                    $atra = 1;
                } else
                    if (($dif >= 2880) and ($dif < 4320)) {
                        $atra = 2;
                    } else
                        if (($dif >= 4320)) {
                            $atra = 3;
    ...
    /* public function detalletxobd2($idu,$cn)
     {
         $hoyf=date("Y-m-d",time());
         $hoy=time();
         $sql="SELECT
    ...
    //--- Función Pública tracking_navef
...
//        $hfin = $nrofilas = $this->input->post('hfx');
        $fini = $this->input->post('fix');
        $ffin = $this->input->post('ffx');
        $hini = $this->input->post('hix');
        $hfin = $this->input->post('hfx');
        ...
        for ($z = 0; $z < $nrofilas; $z++) {
            ...
            if ($this->input->post('cn' . $z)) {
                $valorrr=$this->input->post('cn' . $z);
//                var_dump($_POST,$fini,$ffin,$hini,$hfin,$nrofilas,$valorrr);
//                exit;
                $m++;
                $ditem = $this->input->post('cn' . $z);
                $sql = "SELECT 
    tm_mobil.id_mobil as codigonave, 
      ...
      tm_equipo.id_equipo as id_equipo , 
      id_componente.tipoc ,
      tm_data_mesactual.msg_id as msg_id,
      tm_data_mesactual.msgref as msgref,
        tm_data_mesactual.lat_dd as ddlat, 
        tm_data_mesactual.long_dd as ddlong,
                tm_data_mesactual.lat_dms as latstr, 
        tm_data_mesactual.long_dms as lonstr,
    tm_data_mesactual.gps_date - CAST('5 hour' AS INTERVAL) as Fecha,
      (tm_data_mesactual.speed*1.852)::int as speed,
      tm_data_mesactual.course,
     tm_data_mesactual.ubica as ubica, 
     tm_data_mesactual.provincia as provincia,
     tm_data_mesactual.distrito as distrito,
     tm_data_mesactual.departamento as departamento,
     tm_data_mesactual.panico as panico,
     tm_data_mesactual.fuel_level as  fuel_level,
     tm_data_mesactual.punto_control as  pc,
       ts_tipotx.tipotx as tipotx,
        ts_tipotx.detalle as memcode
    FROM 
      datos.tm_mobil, 
       datos.tm_usuario_mobil, 
      datos.tm_data_mesactual, 
      datos.tm_mobil_equipo, 
      datos.tm_equipo, 
      datos.tm_equipo_componente, 
      datos.id_componente,
      datos.ts_tipotx
    WHERE 
    tm_data_mesactual.gps_date - CAST('5 hour' AS INTERVAL)>'$finix' AND tm_data_mesactual.gps_date - CAST('5 hour' AS INTERVAL) < '$ffinx' AND
    ts_tipotx.tipotx=tm_data_mesactual.tipotx and 
      tm_mobil.id_mobil = tm_data_mesactual.id_mobil AND
    tm_mobil.id_mobil = tm_usuario_mobil.id_mobil AND
    tm_usuario_mobil.id_usuario =  $idx  AND
      tm_mobil.id_mobil = tm_mobil_equipo.id_mobil AND
      tm_equipo.id_equipo = tm_mobil_equipo.id_equipo::int AND
    ...
    //--- Función Pública ultimatxcanbus
    ...
    FROM 
      ...
      datos.id_componente
    WHERE 
    tm_mobil.computadora =1
  --  and tm_mobil.tipounidad=3
     AND   tm_mobil.id_mobil = tm_lastmsg_canbus.id_mobil AND
    tm_mobil.id_mobil = tm_usuario_mobil.id_mobil AND
    tm_usuario_mobil.id_usuario = $idu AND
      tm_mobil.id_mobil = tm_mobil_equipo.id_mobil AND
      tm_equipo.id_equipo = tm_mobil_equipo.id_equipo::int AND
    ...
    //--- Función Pública ultimatxodb2 - 12/12/2017 ---
    public function ultimatxobd2($idu)
    {
        $hoyf = date("Y-m-d", time());
        $hoy = time();
        $sql = "SELECT 
              ...
              tm_equipo.id_equipo as id_equipo , 
              id_componente.tipoc ,
              tm_lastmsg_obd2.latitude as ddlat, 
              tm_lastmsg_obd2.longitude as ddlong,
              to_char(tm_lastmsg_obd2.receivedtime,'DD/MM/YYYY HH24:MI:SS')as Fecha,
              tm_lastmsg_obd2.speed as speed,
              tm_lastmsg_obd2.rpm, 
              tm_lastmsg_obd2.odometer, 
              tm_lastmsg_obd2.fuellevel
              FROM 
              datos.tm_mobil, 
              datos.tm_usuario_mobil, 
              datos.tm_lastmsg_obd2, 
              datos.tm_mobil_equipo, 
              datos.tm_equipo, 
              datos.tm_equipo_componente, 
              datos.id_componente
              WHERE 
              tm_mobil.computadora =2 AND
              tm_mobil.id_mobil = tm_lastmsg_obd2.id_mobil AND
              tm_mobil.id_mobil = tm_usuario_mobil.id_mobil AND
              tm_usuario_mobil.id_usuario = $idu AND
              tm_mobil.id_mobil = tm_mobil_equipo.id_mobil AND
              tm_equipo.id_equipo = tm_mobil_equipo.id_equipo::int AND
              tm_equipo.id_equipo = tm_equipo_componente.id_equipo AND
              tm_equipo_componente.id_com = id_componente.id_com  and estado=1 order by receivedtime desc ; ";
        // echo $sql;
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $row) {
    ...
    //--- Función Pública ultimatxodb2 - 12/12/2017 ---
    public function detalletxobd2($idu)
    {
        $hoyf = date("Y-m-d", time());
        $hoy = time();
        $sql = "SELECT 
                  ...
                  tm_equipo.id_equipo as id_equipo , 
                  id_componente.tipoc ,
                  tm_skywavepollreport_obd2.latitude as ddlat, 
                  tm_skywavepollreport_obd2.longitude as ddlong,
                  tm_skywavepollreport_obd2.returntime - CAST('5 hour' AS INTERVAL) as Fecha,
                  tm_skywavepollreport_obd2.speed as speed,
                  tm_skywavepollreport_obd2.rpm, 
                  tm_skywavepollreport_obd2.odometer, 
                  tm_skywavepollreport_obd2.acceleration,
                  tm_skywavepollreport_obd2.fuellevel
              FROM 
                  datos.tm_mobil, 
                  datos.tm_usuario_mobil, 
                  datos.tm_skywavepollreport_obd2, 
                  datos.tm_mobil_equipo, 
                  datos.tm_equipo, 
                  datos.tm_equipo_componente, 
                  datos.id_componente
              WHERE 
                  tm_mobil.computadora =2 AND
                  tm_skywavepollreport_obd2.obdavailable = 1 AND
                  tm_mobil.id_mobil = tm_skywavepollreport_obd2.id_mobil AND
                  tm_mobil.id_mobil = tm_usuario_mobil.id_mobil AND
                  tm_usuario_mobil.id_usuario = $idu AND
                  tm_mobil.id_mobil = tm_mobil_equipo.id_mobil AND
                  tm_equipo.id_equipo = tm_mobil_equipo.id_equipo::int AND
                  tm_equipo.id_equipo = tm_equipo_componente.id_equipo AND   tm_mobil.id_mobil=7147 and 
                  tm_equipo_componente.id_com = id_componente.id_com  and estado=1  
		  and tm_skywavepollreport_obd2.returntime - CAST('5 hour' AS INTERVAL)>= '2017-12-12 00:00:00' and tm_skywavepollreport_obd2.returntime - CAST('5 hour' AS INTERVAL)<= '2017-12-12 23:59:59'	
    ...
    //--- Función Pública ultimatxcanbusuna
    ...
    //--- Función Pública detalletxcanbus
    ...
    tm_data_canbus_hoy.evttime - CAST('5 hour' AS INTERVAL) as Fecha,
      ...
      tm_data_canbus_hoy.porcent_aceleracion,
      tm_data_canbus_hoy.altitude
    FROM 
      datos.tm_mobil, 
       datos.tm_usuario_mobil, 
...
//    "total_odo" => ($max_odometro-$min_odometro)
                "total_odo" => ($max_odometro - $min_odometro) / 1000
            );
    ...
    /* public function detalletxobd2($idu)
     {
         ...
         if ($query->num_rows() > 0)
         {
             foreach ($query->result() as $row)
             {
                 ...
                 $phpdate = strtotime( date($fecha_gps) );
                 $dif=$hoy-$phpdate;
                 if (($dif>=86400) and ($dif<172800) ) { $atra=1; }
                 else
                     if (($dif>=172800) and ($dif<259200) ) { $atra=2; }
                     else
                         if (($dif>=259200) ) { $atra=3; }
                         else
                         { $atra=0; }
                 $row->atraso=$atra;
                 $data[]=$row;
    ...
    //--- Función Pública detalletxcanbusep
    //mario
    public function detalletxcanbusep($idu, $im, $limite, $motor)
    {
        //$filtrofecha="";
        $filtrofecha = "and tm_data_canbus_hoy.evttime - CAST('5 hour' AS INTERVAL) between '" . date('Y-m-d 00:00:00') . "' and '" . date('Y-m-d H:i:s') . "'";
        $daterange = $this->input->post('daterange');
        $servicios = $this->input->post('seleps');
        $tipo = $this->input->post('tipo');
        if ($tipo) {
        ...
        if ($daterange) {
            $dia_inicio = substr($daterange, 0, 2);
            $mes_inicio = substr($daterange, 3, 2);
            $ano_inicio = substr($daterange, 6, 4);
            $hora_inicio = substr($daterange, 11, 5);
            $fechai = $ano_inicio . '-' . $mes_inicio . '-' . $dia_inicio . ' ' . $hora_inicio;
            $dia_fin = substr($daterange, 19, 2);
            $mes_fin = substr($daterange, 22, 2);
            $ano_fin = substr($daterange, 25, 4);
            $hora_fin = substr($daterange, 30, 5);
            $fechaf = $ano_fin . '-' . $mes_fin . '-' . $dia_fin . ' ' . $hora_fin;
        }
        if ($tipo == 2) {
            $filtrolimite = "limit $limite";
        } else {
        ...
        if ($tipo == 1) {
            $filtrolimite = "";
            //$filtrofecha = "and tm_data_canbus_hoy.evttime- CAST('5 hour' AS INTERVAL)>=to_timestamp('$fechai','DD/MM/YYYY HH24:MI') and tm_data_canbus_hoy.evttime- CAST('5 hour' AS INTERVAL)<=to_timestamp('$fechaf','DD/MM/YYYY HH24:MI')";
            $filtrofecha = "and tm_data_canbus_hoy.evttime-CAST('5 hour' AS INTERVAL) between '$fechai' and '$fechaf'";
        }
        //$hoyf=date("Y-m-d",time());
        //$hoy="2013-09-04";
        //$hoy=time();
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                ...
                //-- Aqui se obtiene el valor maximo del odometro
                $odo = $row->odometer;
                if ($min_odometro > 0) {
                    if ($min_odometro > $odo) {
                        $min_odometro = $odo;
                ...
                //-- Aqui se obtiene Horas totales del motor
                $teh = $row->total_engine_hours;
                if ($min_engine_hours > 0) {
                    if ($min_engine_hours > $teh) {
                        $min_engine_hours = $teh;
                ...
                //-- Aqui se obtiene Combustible total utilizado
                $tfu = $row->total_fuel_used;
                if ($min_total_fuel_used > 0) {
                    if ($min_total_fuel_used > $tfu) {
                        $min_total_fuel_used = $tfu;
    ...
    //--- Función Pública fultimoreporte
    ...
    //--- Función Pública tracking_flotacsv
