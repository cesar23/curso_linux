...
/*
 * fuente:https://zuziko.com/tutorials/how-to-minify-html-in-wordpress-without-a-plugin/
 * */
class FLHM_HTML_Compression
{
    ...
    protected function flhm_bottomComment($raw, $compressed)
    {
        ...
        return '<!--HTML compressed, size saved ' . $savings . '%. From ' . $raw . ' bytes, now ' . $compressed . ' bytes | Creado por: Cesar Auris -->';
    }
...
{
    require(dirname(__FILE__) . '/classSoluciones/FLHM_HTML.php');
    return new FLHM_HTML_Compression($html);
}

function flhm_wp_html_compression_start()
{
// no minimice el HTML en este dominio o parte del dominio
    $blacklist_domain = '.localdev';

// si se llama a la función en un dominio de la lista negra, no minimice el HTML
