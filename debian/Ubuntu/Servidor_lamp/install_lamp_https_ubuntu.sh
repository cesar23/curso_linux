# ejecutar :
# sudo chmod +x install_lamp_https_ubuntu.sh
# sudo install_lamp_https_ubuntu.sh
...
# OPCION 2:  con este no nos pedira configuraciones
openssl req -config "${PATH_CONFIG_CERT}" -new -sha256 -newkey rsa:2048 -nodes -keyout "${PATH_OUT_CERT}/server.key" -x509 -days 365 -out "${PATH_OUT_CERT}/server.crt" -subj "/C=GB/ST=London/L=London/O=Global Security/OU=IT Department/CN=${DOMAIN}"
