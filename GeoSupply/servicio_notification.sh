...
vim /usr/local/bin/noficication_node
    ...
    cd /var/www/geosupply.com.pe/terrestre_mensages
    # cd /home/cesar/project_terrestre_node


    STATUS=$?
    if (( $STATUS!=0 )); then
    logger -p local6.info "ERROR no se  encontro la ubicacion: /var/www/geosupply.com.pe/terrestre_mensages"
    exit
    fi
...
chmod +x /usr/local/bin/noficication_node
...
chmod +x /usr/local/bin/noficication_node_apagado
    #!/bin/bash
    DATE_HOUR="`date +%d`/`date +%m`/`date +%Y` - `date +%H`:`date +%M`:`date +%S`"
...
root@ubuntu:/root# systemctl list-unit-files -t service | grep servicio_notification
				# prueba.service                             disabled
				
root@ubuntu:/root# systemctl start servicio_notification.service

root@ubuntu:/root# systemctl status servicio_notification.service
